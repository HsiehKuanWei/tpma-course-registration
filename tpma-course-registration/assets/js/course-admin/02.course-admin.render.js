(function (w) {
    ...
    ns.buildLecturerFilter = function () {
    ...
    ns.rebuildLecturerSelect = function (sel) {
        if (!sel) return;
    ...
    ns.buildCourseNameFilter = function (courses) {
    ...
    ns.renderCourses = function (list) {
    ...
    ns.renderCourseView = function (div, showAllDates) {
        const c = div._data;
        ...
        try { if (w.marked && outline) { outlineHtml = w.marked.parse(outline); } } catch (e) { /* ignore */ }
    ...
    ns.addSessionRow = function (container, raw) {
        let val = '';
        if (raw) {
    ...
    ns.renderCourseEdit = function (div) {
        const c = div._data;
        ...
        div.innerHTML = `
            ...
            <input type="text"
                   ...
                   value="${util.esc(c.course_code || '')}"
                   placeholder="例如 LEC01-A1">
