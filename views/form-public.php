...
// 動態取得 API 基底：?api_base=... > window.TPMA_API_BASE > fallback
(function () {
  const qsBase = new URLSearchParams(location.search).get("api_base");
  const fallbackA = TPMA_CR_API_BASE;
  const fallbackB = `${location.origin}/wp-json/tpma/v1`;
  window.TPMA_API_BASE = qsBase || window.TPMA_API_BASE || fallbackA || fallbackB;
  if (!window.TPMA_API_BASE) window.TPMA_API_BASE = fallbackB;
})();
const TPMA_API_BASE = window.TPMA_API_BASE;
