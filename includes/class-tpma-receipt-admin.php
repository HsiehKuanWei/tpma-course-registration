...
class TPMA_CR_Receipt_Admin {
    ...
    public static function list_receipts(WP_REST_Request $request) {
        ...
        $receipt_total = self::receipt_list_count($query, $receipt_type, $status);
        $pending_total = self::pending_list_count($query, $receipt_type, $status);
        $total = $receipt_total + $pending_total;
        $offset = ($page - 1) * $per_page;
        $items = array();
        if ($offset < $receipt_total) {
            $receipt_limit = min($per_page, $receipt_total - $offset);
            $items = self::receipt_list_page($query, $receipt_type, $status, $receipt_limit, $offset);
            if (count($items) < $per_page) {
                $items = array_merge($items, self::pending_list_page($query, $receipt_type, $status, $per_page - count($items), 0));
            }
        } else {
            $items = self::pending_list_page($query, $receipt_type, $status, $per_page, $offset - $receipt_total);
        }
    ...
    private static function receipt_list_count(string $query, string $receipt_type, string $status): int {
        global $wpdb;
        list($where, $params) = self::receipt_list_where($query, $receipt_type, $status);
    ...
    private static function receipt_list_page(string $query, string $receipt_type, string $status, int $limit, int $offset): array {
        if ($limit <= 0) {
            return array();
        }
        global $wpdb;
        list($where, $params) = self::receipt_list_where($query, $receipt_type, $status);
        $sql = 'SELECT r.* FROM ' . TPMA_CR_DB::table('receipts') . ' r WHERE ' . implode(' AND ', $where) . ' ORDER BY r.updated_at DESC, r.id DESC LIMIT %d OFFSET %d';
        $params[] = $limit;
        $params[] = $offset;
    ...
    private static function receipt_list_where(string $query, string $receipt_type, string $status): array {
        global $wpdb;
        $where = array('1=1');
        $params = array();
        if ($receipt_type !== '') {
        ...
        if ($query !== '') {
            ...
            $where[] = '(r.serial LIKE %s OR EXISTS (SELECT 1 FROM ' . TPMA_CR_DB::table('receipt_orders') . ' roq WHERE roq.receipt_id=r.id AND CAST(roq.order_id AS CHAR) LIKE %s) OR EXISTS (SELECT 1 FROM ' . TPMA_CR_DB::table('receipt_orders') . ' roq INNER JOIN ' . TPMA_CR_DB::table('regs') . ' rq ON rq.woocommerce_order_id=roq.order_id INNER JOIN ' . TPMA_CR_DB::table('courses') . ' cq ON cq.id=rq.course_id WHERE roq.receipt_id=r.id AND cq.course_name LIKE %s))';
    ...
    private static function pending_list_count(string $query, string $receipt_type, string $status): int {
        if ($status !== '' && $status !== 'pending') {
            return 0;
        }
        global $wpdb;
        list($sql, $params) = self::pending_list_sql($query, $receipt_type);
        $count_sql = 'SELECT COUNT(*) FROM (' . $sql . ') pending_rows';
        return (int) $wpdb->get_var($params ? $wpdb->prepare($count_sql, $params) : $count_sql);
    }

    private static function pending_list_page(string $query, string $receipt_type, string $status, int $limit, int $offset): array {
        ...
        $params[] = $limit;
        $params[] = $offset;
        $rows = $wpdb->get_results($wpdb->prepare($sql . ' ORDER BY order_id DESC LIMIT %d OFFSET %d', $params), ARRAY_A);
        if (!$rows) {
            return array();
        }
    ...
    private static function pending_list_sql(string $query, string $receipt_type): array {
        global $wpdb;
        ...
        if ($query !== '') {
            $like = '%' . $wpdb->esc_like($query) . '%';
            $where[] = '(CAST(r.woocommerce_order_id AS CHAR) LIKE %s OR EXISTS (SELECT 1 FROM ' . TPMA_CR_DB::table('courses') . ' cq WHERE cq.id=r.course_id AND cq.course_name LIKE %s))';
            $params[] = $like;
            $params[] = $like;
        }
        ...
        $sql = 'SELECT r.woocommerce_order_id AS order_id, MAX(CASE WHEN r.receipt_type IN (\'electronic\',\'paper\') THEN r.receipt_type ELSE \'electronic\' END) AS receipt_type FROM ' . $regs . ' r INNER JOIN ' . $order_table['table'] . ' o ON o.' . $order_table['id'] . '=r.woocommerce_order_id WHERE ' . implode(' AND ', $where) . ' GROUP BY r.woocommerce_order_id HAVING ' . $having;
    ...
    public static function bulk_action(WP_REST_Request $request) {
        ...
        if ($action === 'print') {
            return self::stream_batch_pdf(self::normalise_ids($payload['receipt_ids'] ?? array()));
        }
    ...
    public static function render_meta_box($post_or_order): void {
        ...
        $file = TPMA_CR_Receipt_Service::get_effective_file((int) $receipt['id']);
        if (!is_wp_error($file)) {
    ...
    private static function stream_batch_pdf(array $receipt_ids) {
        if (!$receipt_ids) {
            return new WP_Error('tpma_receipt_ids_required', '沒有可列印的收據。', array('status' => 400));
        }
        ...
        $skipped = 0;
        foreach ($receipt_ids as $receipt_id) {
            $file = TPMA_CR_Receipt_Service::get_effective_file($receipt_id);
            if (is_wp_error($file)) {
                $skipped++;
                continue;
            }
            try {
                ...
                return new WP_Error('tpma_receipt_batch_print_failed', '合併列印失敗：' . $e->getMessage(), array('status' => 500));
            }
        }
        if ($included === 0) {
            return new WP_Error('tpma_receipt_file_unavailable', '所選收據尚無可列印的有效檔案。', array('status' => 400));
        }
        try {
            $destination = class_exists('\\Mpdf\\Output\\Destination') ? \Mpdf\Output\Destination::STRING_RETURN : 'S';
            $pdf = $mpdf->Output('', $destination);
        } catch (Throwable $e) {
            return new WP_Error('tpma_receipt_batch_print_failed', '輸出合併收據失敗：' . $e->getMessage(), array('status' => 500));
        }
        header('X-TPMA-Receipt-Skipped: ' . $skipped);
        self::stream_pdf((string) $pdf, 'tpma-receipts-' . wp_date('Ymd-His') . '.pdf', false);
    }
    ...
    private static function render_action_form(int $order_id, string $operation, string $label, string $class = 'button', int $receipt_id = 0): void {
        ...
        echo '<input type="hidden" name="receipt_operation" value="' . esc_attr($operation) . '">';
        if ($receipt_id > 0) {
    ...
    private static function render_scan_form(int $order_id, int $receipt_id): void {
        echo '<form method="post" enctype="multipart/form-data" action="' . esc_url(admin_url('admin-post.php')) . '" style="margin-top:6px;">';
        echo '<input type="hidden" name="action" value="' . esc_attr(self::ACTION) . '">';
        echo '<input type="hidden" name="order_id" value="' . esc_attr((string) $order_id) . '">';
        echo '<input type="hidden" name="receipt_id" value="' . esc_attr((string) $receipt_id) . '">';
        echo '<input type="hidden" name="receipt_operation" value="upload_scan">';
        wp_nonce_field(self::nonce_action($order_id));
        echo '<input type="file" name="receipt_scan" accept="application/pdf,image/jpeg,image/png" required style="max-width:100%;margin-bottom:4px;">';
        echo '<button type="submit" class="button">上傳紙本掃描檔</button></form>';
    }
